...
#define IDEAL_CNT       (8000000-4) // 500 msec  x = 1148
...
long calculations(long xx_input, long yy_input)
{
    ...
    while(!(plus == true && minus == true));    //crossing the 0-line
...
/*		
    ...
    if ((m1 - m2)*(m1 - m2) > ACCURACY_STEP * ACCURACY_STEP)    // testing on noise (14 = 2 * minimum step)
    {
        Setting_Trim(0);    // if out of limits, then TRIM = 0
        return (-1);        // out of limits
    }

    ccc = (m1 + m2)/2;      // calculating avg-value of start-value
*/
		ccc = m1;
