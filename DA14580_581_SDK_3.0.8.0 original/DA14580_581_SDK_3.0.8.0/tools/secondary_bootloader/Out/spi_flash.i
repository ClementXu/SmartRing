#line 1 "..\\..\\dk_apps\\src\\plf\\refip\\src\\driver\\spi_flash\\spi_flash.c"
...
#line 20 "..\\..\\dk_apps\\src\\plf\\refip\\src\\driver\\spi_flash\\spi_flash.c"
...
{
	{0xEF3011, 0xFFFFFF, 0x20000, 0x100, 0x2C, 0},
	{0xEF3012, 0xFFFFFF, 0x40000, 0x100, 0x2C, 0},
	{0x1F4200, 0xFFFF00, 0x20000, 0x100, 4, 0},
};
...
int8_t spi_flash_auto_detect(void)
{
	...
	for (i=0; i<(3); i++)
	{
		if ( (jedec_id & SPI_FLASH_KNOWN_DEVICES_PARAMETERS_LIST[i].jedec_id_matching_bitmask) ==				 (SPI_FLASH_KNOWN_DEVICES_PARAMETERS_LIST[i].jedec_id & SPI_FLASH_KNOWN_DEVICES_PARAMETERS_LIST[i].jedec_id_matching_bitmask) )
...
uint8_t spi_flash_read_status_reg(void)
{
	...
	return spi_transaction((uint16_t)(0x05<<8));
}
...
int8_t spi_flash_wait_till_ready (void)
{
	...
	for (statusReadCount = 0; statusReadCount < 200000; statusReadCount++)
	{
		if ((spi_flash_read_status_reg() & 0x01) == 0)
			return 0;
	}
	return -1;
}
...
int8_t spi_flash_set_write_enable(void)
{
	...
	uint32_t statusReadCount;
	uint8_t status;
	if (spi_flash_wait_till_ready() == 0)
	{
		spi_set_bitmode(SPI_MODE_8BIT);           
		for (commandSendCount = 0; commandSendCount < 50; commandSendCount++)   
		{        
...
int32_t spi_flash_write_status_reg(uint8_t dataToWrite)
{
	...
	if (spi_flash_status != 0)
		return spi_flash_status; 
  
	spi_set_bitmode(SPI_MODE_16BIT);    
	spi_transaction((0x01<<8) | dataToWrite);     
...
int32_t spi_flash_page_program(uint8_t *wr_data_ptr, uint32_t address, uint16_t size)
{
	...
	spi_flash_status = spi_flash_set_write_enable();    
	if (spi_flash_status != 0)
		return spi_flash_status; 						
    
	spi_set_bitmode(SPI_MODE_32BIT);
	spi_cs_low();            			            	
	spi_access( (0x02<<24) | address);        	
	spi_set_bitmode(SPI_MODE_8BIT);           
	while(temp_size>0)                                  
	{
...
int8_t spi_flash_block_erase(uint32_t address, SPI_erase_module_t spiEraseModule)
{
	if (spi_flash_set_write_enable() != 0)         
		return -1;

	spi_set_bitmode(SPI_MODE_32BIT);
	spi_transaction( (spiEraseModule<<24) | address);   
	return spi_flash_wait_till_ready();                 
 }
...
uint64_t spi_read_flash_unique_id(void)
{
	...
	if (spi_flash_status == 0)
	{
		...
		spi_set_bitmode(SPI_MODE_32BIT);           
		spi_access(0x0000);                      
		unique_id = ((uint64_t)spi_access(0x0000) << 32);     
		unique_id |= spi_access(0x0000);          
		spi_cs_high();                           
	}
...
int32_t spi_read_flash_jedec_id(void)
{
	...
	spi_cs_low();            		   	  
	spi_access(0x9f);                 
	jedec_id = spi_access(0x0000) << 16;  
	jedec_id |= spi_access(0x0000) << 8;  
	jedec_id |= spi_access(0x0000);       
	spi_cs_high();                        
	return jedec_id;	   
}
...
int32_t spi_flash_write_data (uint8_t *wr_data_ptr, uint32_t address, uint32_t size)
{
	...
	while (bytes_written < feasible_size)
	{
		...
		if (spi_flash_page_program(wr_data_ptr + bytes_written, currentAddress, bytes_left_to_send) != 0) 
			return -1;
...
int32_t spi_flash_configure_memory_protection(uint8_t spi_flash_memory_protection_setting)
{
	...
	return spi_flash_write_status_reg((spi_flash_read_status_reg() & (~spi_flash_detected_device->memory_protection_bitmask)) |		((spi_flash_memory_protection_setting)&(spi_flash_detected_device->memory_protection_bitmask)));        
...
int32_t spi_flash_fill (uint8_t value, uint32_t address, uint32_t size)
{
	...
	while (bytes_written < feasible_size)
	{
		...
		if (currentAddress + bytes_left_to_send > currentEndOfPage)
			bytes_left_to_send = currentEndOfPage - currentAddress + 1;             
		if (spi_flash_page_fill(value, currentAddress, bytes_left_to_send) != 0) 
			return -1;
		bytes_written += bytes_left_to_send;                                                     
		currentAddress = currentEndOfPage + 1;  
		currentEndOfPage += spi_flash_page_size;
