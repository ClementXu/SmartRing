    ...
    while(1) {
        ...
        switch(action) 
        {
                ...
                break;
						...
						case ACTION_UART_GPIOS:
								...
								if (result == ERR_OK) {
									...
									GPIO_ConfigurePin( (GPIO_PORT) uart_sel_pins.uart_port, (GPIO_PIN) uart_sel_pins.uart_tx_pin, INPUT, PID_GPIO, false);
									GPIO_ConfigurePin( (GPIO_PORT) uart_sel_pins.uart_port, (GPIO_PIN) uart_sel_pins.uart_rx_pin, INPUT, PID_GPIO, false);
									uart_sel_pins.uart_port = buffer[1];
									uart_sel_pins.uart_tx_pin = buffer[2];
									uart_sel_pins.uart_rx_pin = buffer[3];
									set_pad_uart();
								}
								break;						
            ...
            case ACTION_SPI_GPIOS:
								result = ERR_OK;
								// port should be the same for all pins
								if ((buffer[1] & buffer[3] & buffer[5] & buffer[7]) != buffer[1])
									result = -1;
								...
								if (result == ERR_OK) {
									spi_sel_pins.spi_cs_port = buffer[1];
									spi_sel_pins.spi_cs_pin = buffer[2];
									spi_sel_pins.spi_clk_port = buffer[3];
									spi_sel_pins.spi_clk_pin = buffer[4];
									spi_sel_pins.spi_do_port = buffer[5];
									spi_sel_pins.spi_do_pin = buffer[6];
									spi_sel_pins.spi_di_port = buffer[7];
									spi_sel_pins.spi_di_pin = buffer[8];
								}
								response_action(buffer, result, port_sel);
								break;
						...
						case ACTION_I2C_GPIOS:
								...
								if (result == ERR_OK) {
									i2c_sel_pins.i2c_scl_port = buffer[1];
									i2c_sel_pins.i2c_scl_pin = buffer[2];
									i2c_sel_pins.i2c_sda_port = buffer[3];
									i2c_sel_pins.i2c_sda_pin = buffer[4];
								}
								response_action(buffer, result, port_sel);
								break;
