...
void app_send_pairing_rsp_func(struct gapc_bond_req_ind *param)
{
    ...
    // OOB information
    cfm->data.pairing_feat.oob            = GAP_OOB_AUTH_DATA_PRESENT;
    // Encryption key size
...
struct gapc_bond_cfm *p_cfm ;
void app_send_tk_exch_func(struct gapc_bond_req_ind *param)
{
    
    uint8_t oob_tk[KEY_LEN] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f };
    struct gapc_bond_cfm* cfm = KE_MSG_ALLOC(GAPC_BOND_CFM, KE_BUILD_ID(TASK_GAPC, app_env.conidx), TASK_APP, gapc_bond_cfm);
    p_cfm = cfm;
    memcpy(cfm->data.tk.key, oob_tk, KEY_LEN);
