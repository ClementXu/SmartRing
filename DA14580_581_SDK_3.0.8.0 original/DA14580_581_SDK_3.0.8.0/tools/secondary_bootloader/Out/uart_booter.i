...
#line 21 "src\\uart_booter.c"
#line 22 "src\\uart_booter.c"
...
int FwDownload(void)
{
    ...
    if (0 == uart_receive_byte(&recv_byte)) return -1;  
    if (recv_byte != 0x01) return -2;

    if (0 == uart_receive_byte(&recv_byte)) return -3;  
    fw_size = recv_byte;

    if (0 == uart_receive_byte(&recv_byte)) return -4;  
    fw_size += (recv_byte << 8);
    ...
    for (i = 0; i < fw_size; i++)                       
    {
        if (0 == uart_receive_byte(&recv_byte)) return -6; 
        crc_code ^= recv_byte;                          
        *code = recv_byte;                              
        code++;                                         
    }
    uart_send_byte(crc_code);                           

    if (0 == uart_receive_byte(&recv_byte)) return -7;  
    if (recv_byte != 0x06) return -8;
